...
void CreatePool()
{
	...
	for (int xOsc = 0; xOsc < NUM_X_OSCILLATORS; xOsc++) 
		for (int zOsc = 0; zOsc < NUM_Z_OSCILLATORS; zOsc++) 
		{
			Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].x = OSCILLATOR_DISTANCE*float(xOsc);
			Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].y = 0.0f;
			Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].z = OSCILLATOR_DISTANCE*float(zOsc);

			Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].nx = 0.0f;
			Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].ny = 1.0f;
			Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].nz = 0.0f;

			Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].UpSpeed = 0;
			Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].bIsExciter = false;

			//create two triangles:
			if ((xOsc < NUM_X_OSCILLATORS-1) && (zOsc < NUM_Z_OSCILLATORS-1))
			{
				IndexVect.push_back(xOsc+zOsc*NUM_X_OSCILLATORS);
				IndexVect.push_back((xOsc+1)+zOsc*NUM_X_OSCILLATORS);
				IndexVect.push_back((xOsc+1)+(zOsc+1)*NUM_X_OSCILLATORS);

				IndexVect.push_back(xOsc+zOsc*NUM_X_OSCILLATORS);
				IndexVect.push_back((xOsc+1)+(zOsc+1)*NUM_X_OSCILLATORS);
				IndexVect.push_back(xOsc+(zOsc+1)*NUM_X_OSCILLATORS);
			}
...
// PLEASE NOTE: THESE ARE APPROXIMATIONS AND I KNOW THIS! (but is looks good, doesn't it?)
	...
	for (xOsc = 0; xOsc < NUM_X_OSCILLATORS; xOsc++) 
	{
		for (zOsc = 0; zOsc < NUM_Z_OSCILLATORS; zOsc++) 
		{
			int ArrayPos = xOsc+zOsc*NUM_X_OSCILLATORS;
			...
			if ((xOsc==0) || (xOsc==NUM_X_OSCILLATORS-1) || (zOsc==0) || (zOsc==NUM_Z_OSCILLATORS-1))
	...
	for ( xOsc = 0; xOsc < NUM_X_OSCILLATORS; xOsc++) 
	{
		for (zOsc = 0; zOsc < NUM_Z_OSCILLATORS; zOsc++) 
		{
			Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].y =Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].newY;
		}
	}
	...
	for ( xOsc = 0; xOsc < NUM_X_OSCILLATORS; xOsc++) 
	{
		for (zOsc = 0; zOsc < NUM_Z_OSCILLATORS; zOsc++) 
		{
			...
			if (xOsc > 0) p1 = F3dVector(Oscillators[xOsc-1+zOsc*NUM_X_OSCILLATORS].x,
									   Oscillators[xOsc-1+zOsc*NUM_X_OSCILLATORS].y,
									   Oscillators[xOsc-1+zOsc*NUM_X_OSCILLATORS].z);
			else
						p1 = F3dVector(Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].x,
									   Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].y,
									   Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].z);	
			if (xOsc < NUM_X_OSCILLATORS-1) 
						p2 = F3dVector(Oscillators[xOsc+1+zOsc*NUM_X_OSCILLATORS].x,
									   Oscillators[xOsc+1+zOsc*NUM_X_OSCILLATORS].y,
									   Oscillators[xOsc+1+zOsc*NUM_X_OSCILLATORS].z);
			else
						p2 = F3dVector(Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].x,
									   Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].y,
									   Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].z);	
			u = p2-p1; //vector from the left neighbor to the right neighbor
			if (zOsc > 0) p1 = F3dVector(Oscillators[xOsc+(zOsc-1)*NUM_X_OSCILLATORS].x,
									   Oscillators[xOsc+(zOsc-1)*NUM_X_OSCILLATORS].y,
									   Oscillators[xOsc+(zOsc-1)*NUM_X_OSCILLATORS].z);
			else
						p1 = F3dVector(Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].x,
									   Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].y,
									   Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].z);	
			if (zOsc < NUM_Z_OSCILLATORS-1) 
						p2 = F3dVector(Oscillators[xOsc+(zOsc+1)*NUM_X_OSCILLATORS].x,
									   Oscillators[xOsc+(zOsc+1)*NUM_X_OSCILLATORS].y,
									   Oscillators[xOsc+(zOsc+1)*NUM_X_OSCILLATORS].z);
			else
						p2 = F3dVector(Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].x,
			...
			//assign the normal:
			Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].nx = normal.x;
			Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].ny = normal.y;
			Oscillators[xOsc+zOsc*NUM_X_OSCILLATORS].nz = normal.z;
		}
	}
